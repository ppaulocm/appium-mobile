Feature: Change nickname
    ...
    Scenario: Login using facebook account
        ...
        Then a dialog box open to edit user nickname
        When player insert a new nickname
            And tap on validate button
        Then a message indicating nickname availability is displayed
        When player tap on Save
//Then new nickname must appear on top of screen
